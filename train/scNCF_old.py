...
import scanpy as sc
from data.data_utils import prepare_data2,prepare_data3, collate_train, collate_val,collate_denoise
...
def scNCF(adata,
    ...
    assert encoder in ['GMF', 'MLP', 'NeuCF'], "Please choose model in ['GMF', 'MLP', 'NeuCF']"
    ...
    if n_clusters is None:
        if cell_type:
            ...
            print('Please input number of clusters or set cell type information in adata[\'cl_type\']')
        ...
        for train_data in train_dataloader:
            ...
            if decoder == 'ZINB':
                ...
                cell_idx, exp_gene_idx, exp_value, unexp_gene_idx, sz_factor, exp_ge_factor, unexp_ge_factor = \
                    torch.from_numpy(cell_idx).to(device),torch.from_numpy(exp_gene_idx).to(device), torch.from_numpy(exp_value).float().to(device),\
                    torch.from_numpy(unexp_gene_idx).to(device),torch.from_numpy(sz_factor).float().to(device),\
                ...
                cell_idx, exp_gene_idx, exp_value, unexp_gene_idx = torch.from_numpy(cell_idx).to(device),torch.from_numpy(exp_gene_idx).to(device), \
                                                                    torch.from_numpy(exp_value).float().to(device),torch.from_numpy(unexp_gene_idx).to(device)
    ...
    cell_idx_list, gene_idx_list = [], []
    mu_list, disp_list, pi_list = [], [], []
    # mu_unexp_list,exp_gene_idx_list,unexp_gene_idx_list= [], [], []
    pred_value_list = []
    # after training ,to impute matrix
    if mode == 'denoise':
        print("Start imputing!")
        adata, denoise_datasets = prepare_data3(adata, decoder, raw_exp)
        denoise_dataloader = DataLoader(denoise_datasets, batch_size=batch_size, shuffle=True,
                                        drop_last=False, num_workers=numworker, collate_fn=collate_denoise)
        for data in denoise_dataloader:
            if decoder == 'ZINB':
                cell_idx, gene_idx, value, sz_factor, ge_factor = data
                cell_idx, gene_idx, value, sz_factor, ge_factor \
                    = torch.from_numpy(cell_idx).to(device), torch.from_numpy(gene_idx).to(device), \
                      torch.from_numpy(value).float().to(device), torch.from_numpy(sz_factor).float().to(device), \
                      torch.from_numpy(ge_factor).float().to(device)

                pred_exp = model(cell_idx, gene_idx, sz_factor, ge_factor)
                mu_list.extend(pred_exp[0].tolist())
                cell_idx_list.extend(cell_idx.tolist())
                gene_idx_list.extend(gene_idx.tolist())

            else:
                cell_idx, gene_idx, value = data
                cell_idx, gene_idx, value = torch.from_numpy(cell_idx).to(device), \
                                            torch.from_numpy(gene_idx).to(device), torch.from_numpy(value).float().to(device)

                pred_exp = model(cell_idx, gene_idx)
                pred_value_list.extend(pred_exp.tolist())
                cell_idx_list.extend(cell_idx.tolist())
                gene_idx_list.extend(gene_idx.tolist())
        ...
        if decoder == 'ZINB':
            ...
            cell_idx_list = np.array(cell_idx_list)
            gene_idx_list = np.array(gene_idx_list)
            mu_list = mu_list.flatten()
            cell_idx_list = cell_idx_list.flatten()
            gene_idx_list = gene_idx_list.flatten()
            imputed = csr_matrix((mu_list, (cell_idx_list, gene_idx_list)), shape=(M, N))
            imputed_matrix = imputed.todense()
        else:
            pred_value_list = np.array(pred_value_list)
            cell_idx_list = np.array(cell_idx_list)
            gene_idx_list = np.array(gene_idx_list)
            pred_value_list = pred_value_list.flatten()
            cell_idx_list = cell_idx_list.flatten()
            gene_idx_list = gene_idx_list.flatten()
            imputed = csr_matrix((pred_value_list, (cell_idx_list, gene_idx_list)), shape=(M, N))
            X_imputed = np.array(imputed.todense())

            gene_factor = adata.var['gene_factor'].values
            X_imputed = X_imputed * gene_factor
            adata.obsm['X_imputed_processed'] = X_imputed

            XPred = np.expm1(X_imputed)
            size_factor = adata.obs['size_factor'].values
            imputed_matrix = np.dot(np.diag(size_factor), XPred)
